...
## Additions Compared With Upstream Career-Ops

I compared this fork against `santifer/career-ops` and found several categories of work that are not part of the original baseline. The short version: upstream Career-Ops is the core job-search engine; this fork adds operator tooling, automation wrappers, dashboard infrastructure, extra scanner layers, public demo data, and multi-agent command adapters.

| Area added or changed | Files / examples | What it does | Why it was implemented | How it helps me |
|---|---|---|---|---|
| Hermes operations layer | external `career-ops-operations` skill, documented in this README | Gives Hermes a runbook for setup checks, scans, tracker rules, dashboards, email ingest, PDFs, and public-release safety | The original repo has scripts and modes, but not a long-lived operational memory for running them safely day after day | Reduces repeated manual instructions and makes Hermes act like an operator that knows my Career-Ops environment |
| Multi-model orchestration docs | `docs/HERMES-MULTI-MODEL-SETUP.md`, `docs/CODEX.md` | Documents Hermes as coordinator, ChatGPT/Codex as reasoning/review, and Claude Code as bounded worker | I needed the repo to explain how my actual AI stack works, not just the upstream Claude-centric workflow | Makes the public repo honest about how I got Hermes, Claude, and ChatGPT/Codex working together |
| Claude slash command adapters | `.claude/commands/career-ops*.md`, `.claude/commands/dashrestart.md`, `.claude/commands/restartreact.md` | Adds explicit Claude Code commands for evaluate, scan, pipeline, PDF, tracker, apply, batch, follow-up, patterns, and dashboard restarts | The original repo expected agent context, but this fork gives Claude concrete command entrypoints | Lets me trigger common Career-Ops workflows quickly and consistently from Claude Code |
| Gemini command adapters | `.gemini/commands/*.toml`, `GEMINI.md` | Mirrors Career-Ops commands for Gemini CLI | I wanted the workflow to be CLI/model-agnostic instead of locked to one AI tool | Gives me another low-cost or fallback model interface while keeping the same modes and data files |
| OpenCode command adapters | `.opencode/commands/*.md` | Adds command wrappers for OpenCode | Same reason as Gemini: multiple agent CLIs can operate the same repo | Makes the system more portable if I switch tools or want to test another coding agent |
...
| React web dashboard | `web-dashboard.react.mjs`, `mock/*.jsx`, `mock/Career Ops Dashboard.html` | Adds a component-based dashboard variant with calendar, pipeline, progress, scan queue, drawer, tweaks panel, and Kanban components | I wanted a cleaner, more modular UI for iterating on dashboard features | Makes it easier to improve layout and test new UI ideas without touching the original all-in-one dashboard |
| Dashboard action wrappers | `scan-run.mjs`, `pipeline-run.mjs` | Lets dashboard buttons invoke Career-Ops scan and pipeline flows through Claude Code | Browser buttons need a controlled way to trigger agent workflows | Turns the dashboard from read-only status into an action surface |
| Interview-stage parsing references | `references/interview-stage-parsing-*.md`, `web-dashboard-lib.mjs` | Adds patterns for extracting interview stage/date/interviewer metadata from tracker notes | Interview progress is messy and often encoded in notes | Helps dashboards and Hermes show where I am in each interview process |
| Screening response renderer | `render-screening-responses.mjs` | Converts markdown screening responses into styled PDFs through Playwright | Application forms often need polished response packets, and I prefer final PDFs | Lets me produce clean PDF deliverables for screening questionnaires |
| Interview-prep/PDF render helpers | `scripts/md-to-pdf-prep*.mjs`, changes to `modes/interview-prep.md` | Converts prep notes into PDF-friendly formats, including mobile-oriented prep docs | Interview prep needs to be readable and portable, not just markdown in a repo | Gives me final prep documents I can actually use before calls |
| CV/profile variants | `templates/cv-exec-support.md`, `cv-it-engineer.md`, `cv-it-ops-manager.md`, `cv-solutions-engineer.md`, `templates/cv-variants.html` | Adds fictional public-safe resume variants aligned to support, IT engineering, ops management, and solutions roles | My local workflow needed role-specific framing, but public examples cannot expose my real resume | Shows the customization pattern while keeping private resume data out of GitHub |
| Public dashboard demo data | `examples/dashboard-demo/**` | Adds fictional tracker, pipeline, reports, prep notes, profile, and placeholder PDFs | A public repo needs a quick way for others to test the dashboard without my private data | Lets reviewers see the UI and data flow safely in minutes |
| macOS Finder/NFS helper | `install-career-ops-finder-mounts.sh` | Mounts selected Career-Ops folders as Finder-visible volumes via macOS NFS | Generated outputs, images, notes, and prep docs are easier to browse from Finder | Makes the local workflow more usable on macOS |
| Watchers | `watchers/midjourney-careers.mjs`, `fetch-jd-tmp.mjs` | Adds targeted page watching / JD fetch helpers for sources that do not fit direct API scans | Some sources need Playwright/page-specific extraction instead of generic APIs | Captures opportunities that API-only scanning would miss |
| Public cleanup / sanitization | `.gitignore`, deleted private example CV/story-bank files, sanitized README/docs | Removes or ignores private runtime data and upstream-personal material | A public fork cannot leak real CVs, trackers, reports, interview prep, or credentials | Lets me publish the work safely while preserving the architecture and reusable code |
