...
> **Nota (v1.8+):** el flujo recomendado de `career-ops scan` / `npm run scan` ahora ejecuta DOS capas zero-token / low-token en secuencia: (1) `scan.mjs` para `tracked_companies` con providers soportados, y (2) `scan-job-boards.mjs` para `job_board_integrations` como JSearch, Adzuna, Ever Jobs y otros agregadores configurados. Si una empresa no tiene provider soportado en `scan.mjs`, aún puede entrar por la capa de job boards si está configurada allí.

> **Ever Jobs add-on:** Ever Jobs ya forma parte de la segunda capa (`scan-job-boards.mjs`). Para usarlo, deja `job_board_integrations.ever_jobs.enabled: true` y levanta su API local en `http://localhost:3001` antes del scan.
