...
function isIntentionalReapplication(group) {
  if (group.length !== 2) return false;
  const notes = group.map(e => (e.notes || '').toLowerCase()).join(' ');
  const statuses = group.map(e => (e.status || '').toLowerCase());
  const hasPriorClosedOutcome = statuses.some(s => s.includes('rejected') || s.includes('discarded') || s.includes('skip'));
  const hasNewPostingSignal = /new posting|different req|different requisition|reapplication|encouraged reapplication|encouraged to apply/.test(notes);
  return hasPriorClosedOutcome && hasNewPostingSignal;
}
...
for (const [key, group] of companyRoleMap) {
  if (group.length > 1) {
    const refs = group.map(e => `#${e.num}`).join(', ');
    if (isIntentionalReapplication(group)) {
      ok(`Intentional reapplication noted, not treated as duplicate: ${refs} (${group[0].company} — ${group[0].role})`);
      continue;
    }
    warn(`Possible duplicates: ${refs} (${group[0].company} — ${group[0].role})`);
    dupes++;
  }
}
